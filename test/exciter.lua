...
do
  ...
  check("not gated with only an S-socket cable", first.flag == 0, tostring(first.flag))
...
print("\n-- S -> the M socket, through the patch matrix --")
do
  local M = fresh(1)
  local BECK = "s.beck"
  ...
  check("dst is Oak's mod-in bus", add.dst == M.bridge.bus("mod_in", oak.index - 1))
  check("gain matches edge gain", add.gain == 0.5)

  local eid = M.patch.has(BECK, "oak.mod")
...
print("\n-- P<->M is a legal cable with no continuous meaning --")
do
  local M = fresh(1)
  local ok = pcall(function() M.patch.add("oak.pitch", "oak.mod", 0.5) end)
  check("cabling still works", ok and M.patch.has("oak.pitch", "oak.mod") ~= nil)
