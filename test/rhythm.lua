...
local function two_drifters(seed, gain, scatter, seconds)
  local M = fresh(seed)
  -- Scatter is what this test sweeps; Swing used to come along for free as the
...
-- Scatter 0 zeroes the drift random walk, so these are deterministic.
...
do
  ...
  check("still locks with Scatter up", hi < 0.15, string.format("gap %.3f..%.3f", lo, hi))
end
