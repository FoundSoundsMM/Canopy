-- the re-cut's re-cut (docs/canopy-spec.md §2.7b): the six G percussion
-- cells. that they're registered right (three ping, three noise, in the
-- middle of the bottom weave row), that the sound page (lib/gvoice.lua)
-- pushes and nudges like the four corner voices' does, that a pulse strikes
-- one the same way it strikes a voice's T socket (refractory and all), and
-- that a G cell answers with a pulse of its own without sending it back down
-- the cable it arrived on.
...
local YAFFLE, KNAP, SCREE = "g.yaffle", "g.knap", "g.scree"
...
print("\n-- six G cells, three ping and three noise, in the bottom row --")
do
  local M = fresh(1)
  local ids = {}
  for id, cell in M.topology.each() do
    if cell.type == "G" then table.insert(ids, id) end
  end
  ...
  for _, id in ipairs(ids) do
    ...
    check(id .. " sits in row 7", c.coords[1][2] == 7, tostring(c.coords[1][2]))
  end
...
do
  local M = fresh(3)
  M.dispatch.on_pulse("d.knocker", YAFFLE, {gain = 0.8}, 1.0)
...
do
  local M = fresh(3)
  M.dispatch.on_pulse("d.knocker", KNAP, {gain = 1.0}, 1.0)
  M.dispatch.on_pulse("d.knocker", KNAP, {gain = 1.0}, 1.0)
...
do
  ...
  M.patch.add(SCREE, BECK, 1.0)
  M.patch.add(SCREE, LOAM, 1.0)
  run(M, 3)
